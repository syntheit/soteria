...
import {collection, doc, setDoc, getDoc, DocumentReference, DocumentData, addDoc, Timestamp} from "firebase/firestore";
...
const NewPost: NextPage<Props> = () => {
    ...
    const makeUserPost = async (postsPath: string, uid: string) => {
        ...
        await setDoc(newDoc, {
            ...
            start_time: Timestamp.fromDate(new Date(now.getFullYear(), now.getMonth(), now.getDate(), Number(startTime.substring(0, 2)), Number(startTime.substring(3)))),
            end_time: Timestamp.fromDate(new Date(now.getFullYear(), now.getMonth(), now.getDate(), Number(endTime.substring(0, 2)), Number(endTime.substring(3)))),
        ...
        if (images && images instanceof FileList) {
            Array.from(images).forEach(file => uploadBytes(ref(storage, 'images/' + uid + '/' + newDoc.id + '/' + file.name), file).then((snapshot) => {
